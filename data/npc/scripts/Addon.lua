...
already        = 'It seems you already have this addon, don\'t you try to mock me son!' 
	  ...
	  -- HAND OF INQUISITION
function handofinqFirst(cid, message, keywords, parameters, node) 
    ...
    addon = getPlayerStorageValue(cid,handofinq) 
    if addon == -1 then 
        if getPlayerItemCount(cid,36573) >= 1 then 
        if doPlayerRemoveItem(cid,36573,1) then 
            ...
            setPlayerStorageValue(cid,handofinq,1) 
        end 
        else 
            selfSay(noitems, cid) 
        end 
...
function handofinqSecond(cid, message, keywords, parameters, node)
    ...
    if isPremium(cid) then 
    addon = getPlayerStorageValue(cid,handofinq+1) 
    if addon == -1 then 
        if getPlayerItemCount(cid,36572) >= 1 then 
        if doPlayerRemoveItem(cid,36572,1) then 
            ...
            doPlayerAddOutfit(cid, 1243, 2) 
            setPlayerStorageValue(cid,handofinq+1,1) 
        end 
        else 
            selfSay(noitems, cid) 
        end 
...
keywordHandler:addKeyword({'addons'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = "I can offer you first & second addons of the following outfits: Citizen, Hunter, Mage, Knight, Nobleman, Summoner, Warrior, Barbarian, Druid, Wizard, Oriental, Pirate, Assassin, Beggar, Shaman, Norseman, Elementalist, Nightmare, Jester, Brotherhood and Hand of Inquisition."}) 
...
-- elementalist end
-- hand of inquisition
    node51 = keywordHandler:addKeyword({'first hand of inquisition addon'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To achieve the first Hand of inquisition addon you need to give me 1 Final Judgement. Do you have them with you?'}) 
    node51:addChildKeyword({'yes'}, handofinqFirst, {}) 
    ...
    node52 = keywordHandler:addKeyword({'second hand of inquisition addon'}, StdModule.say, {npcHandler = npcHandler, onlyFocus = true, text = 'To achieve the second Hand of inquisition addon you need to give me 1 Shadow Cowl. Do you have them with you?'}) 
    node52:addChildKeyword({'yes'}, handofinqFirst, {}) 
