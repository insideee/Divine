local config = {
	requiredLevel = 100,
	daily = true,
	roomCenterPosition = Position(33455, 31436, 13),
	playerPositions = {
	...
	bossPosition = Position(33456, 31443, 13)
}
	...
	if item.itemid == 9825 then
		...
		for i = 1, #config.playerPositions do
			...
			if participant and participant:isPlayer() then
				...
				if config.daily and participant:getStorageValue(Storage.GraveDanger.CountVlarkorthTimer) > os.time() then
		...
		for i = 1, #team do
			...
			-- Assign boss timer
			team[i]:setStorageValue(Storage.GraveDanger.CountVlarkorthTimer, os.time() + 20*60*60) -- 20 hours
		end
