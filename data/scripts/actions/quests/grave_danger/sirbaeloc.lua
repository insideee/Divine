local config = {
	requiredLevel = 100,
	daily = true,
	roomCenterPosition = Position(33424, 31440, 13),
	playerPositions = {
	...
	teleportPosition = Position(33420, 31447, 13),
	bossPosition = Position(33424, 31435, 13)
}
	...
	if item.itemid == 9825 then
		...
		for i = 1, #config.playerPositions do
			...
			if participant and participant:isPlayer() then
				...
				if config.daily and participant:getStorageValue(Storage.GraveDanger.SirBaelocTimer) > os.time() then
		...
		for i = 1, #team do
			...
			-- Assign boss timer
			team[i]:setStorageValue(Storage.GraveDanger.SirBaelocTimer, os.time() + 20*60*60) -- 20 hours
		end
