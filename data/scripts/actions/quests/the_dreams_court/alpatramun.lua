local config = {
	requiredLevel = 100,
	daily = true,
	roomCenterPosition = Position(32176, 32044, 15),
	playerPositions = {
	...
	teleportPosition = Position(32177, 32052, 15),
	bossPosition = Position(32175, 32037, 15)
}
	...
	if item.itemid == 10030 then
		...
		for i = 1, #config.playerPositions do
			...
			if participant and participant:isPlayer() then
				...
				if config.daily and participant:getStorageValue(Storage.AlptramunTime) > os.time() then
		...
		for i = 1, #team do
			...
			-- Assign boss timer
			team[i]:setStorageValue(Storage.AlptramunTime, os.time() + 20*60*60) -- 20 hours
		end
