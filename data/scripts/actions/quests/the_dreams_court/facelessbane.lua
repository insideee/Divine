local config = {
	requiredLevel = 100,
	daily = true,
	roomCenterPosition = Position(33618, 32563, 13),
	playerPositions = {
	...
	teleportPosition = Position(33625, 32562, 13),
	bossPosition = Position(33609, 32562, 13)
}
	...
	if item.itemid == 9825 then
		...
		for i = 1, #config.playerPositions do
			...
			if participant and participant:isPlayer() then
				...
				if config.daily and participant:getStorageValue(Storage.FacelessBaneTime) > os.time() then
		...
		for i = 1, #team do
			...
			-- Assign boss timer
			team[i]:setStorageValue(Storage.FacelessBaneTime, os.time() + 20*60*60) -- 20 hours
		end
