local config = {
	requiredLevel = 250,
	daily = true,
	roomCenterPosition = Position(32208, 32045, 15),
	playerPositions = {
	...
	teleportPosition = Position(32206, 32052, 15),
	bossPosition = Position(32209, 32038, 15)
}
	...
	if item.itemid == 9825 then
		...
		for i = 1, #config.playerPositions do
			...
			if participant and participant:isPlayer() then
				...
				if config.daily and participant:getStorageValue(Storage.NightmareTime) > os.time() then
		...
		for i = 1, #team do
			...
			-- Assign boss timer
			team[i]:setStorageValue(Storage.NightmareTime, os.time() + 20*60*60) -- 20 hours
		end
